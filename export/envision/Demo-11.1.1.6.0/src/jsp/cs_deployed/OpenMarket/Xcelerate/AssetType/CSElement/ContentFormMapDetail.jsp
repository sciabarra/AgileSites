...
<%//
// OpenMarket/Xcelerate/AssetType/CSElement/ContentFormMapDetail
//
// INPUT
//
// OUTPUT
...
<cs:ftcs>
<%-- Create list of all typeless template before hand --%>
<%
...
<ics:if condition='<%="true".equals(ics.GetVar("showSiteTree")) || "ucform".equals(ics.GetVar("cs_environment"))%>'><ics:then>
    ...
    <script type="text/javascript">
        ...
        {
            var typeDropDown = document.forms[0].elements['CSElement:Mapping:'+ counter +':type'];
            ...
            document.forms[0].elements['CSElement:Mapping:'+ counter +':selectedid'].value='';
            document.forms[0].elements['CSElement:Mapping:'+ counter +':selectedtype'].value='';
            document.forms[0].elements['CSElement:Mapping:'+ counter +':selectedname'].value='';

            //select asset and place in hidden fields
			<%if("ucform".equals(ics.GetVar("cs_environment"))) { %>
				 SelectFromTreeTypedTextField('CSElement:Mapping:'+ counter +':selectedname','CSElement:Mapping:'+ counter +':selectedid','CSElement:Mapping:'+ counter +':selectedtype',legalTypes,'_typeAheadElementMapping_'+counter );
			<%}else{%>
				 SelectFromTreeTypedTextField('CSElement:Mapping:'+ counter +':selectedname','CSElement:Mapping:'+ counter +':selectedid','CSElement:Mapping:'+ counter +':selectedtype',legalTypes,'single');
			<%}%>
           
			//Verify that selectfromtreetypetextfield returned a valid asset.
            if (document.forms[0].elements['CSElement:Mapping:'+ counter +':selectedid'].value.length==0)
                return false;
            ...
            if (mappingType=="tname")
            {
                ...
                var selectedtid = document.forms[0].elements['CSElement:Mapping:'+ counter +':selectedid'].value;
                if (arrayContains(totaltltids, selectedtid))
                {
                    document.forms[0].elements['CSElement:Mapping:'+ counter +':value'].value =
                    '/'+document.forms[0].elements['CSElement:Mapping:'+ counter +':selectedname'].value;
                }
                else
                {
            ...
            {
                tempValue = document.forms[0].elements['CSElement:Mapping:'+ counter +':selectedtype'].value+":"+document.forms[0].elements['CSElement:Mapping:'+ counter +':selectedname'].value;
				document.forms[0].elements['CSElement:Mapping:'+ counter +':value'].value = tempValue;
            }
            if (mappingType=="asset")
            {
                tempValue = document.forms[0].elements['CSElement:Mapping:'+ counter +':selectedtype'].value+":"+document.forms[0].elements['CSElement:Mapping:'+ counter +':selectedid'].value;
                document.forms[0].elements['CSElement:Mapping:'+ counter +':value'].value = tempValue;
            }
        }
    </script>
...
<script type="text/javascript">
    ...
    function showMappingForSelectedSite()
    {
        ...
        document.forms[0].elements['<%="CSElement:Mapping:"+(Integer.parseInt(ics.GetVar("total"))-1)+":siteid"%>'].value=selectedsiteid;
    }
	</script>
...
%>
    ...
    <tr>
		...
		<ics:then>			
			...
			<INPUT TYPE="TEXT" NAME='<%= "CSElement:Mapping:" + curCount + ":key"%>' VALUE='<string:stream variable="CKey"/>' SIZE="32" MAXLENGTH="32"/>
		</ics:then>
		<ics:else>
			<INPUT TYPE="TEXT" NAME='<%= "CSElement:Mapping:" + curCount + ":key"%>' VALUE="" SIZE="32" MAXLENGTH="32"/>
		</ics:else>
		</ics:if>
		</td>
		...
		<td>
<%
		String typename = "CSElement:Mapping:" + curCount + ":type";
%>		
...
<%
		...
		<ics:then>			
			...
			<INPUT TYPE="TEXT" NAME='<%= "CSElement:Mapping:" + curCount + ":value"%>' VALUE='<string:stream variable="CVALUE"/>' SIZE="32" MAXLENGTH="256"/>
		</ics:then>
		<ics:else>
			<INPUT TYPE="TEXT" NAME='<%= "CSElement:Mapping:" + curCount + ":value"%>' VALUE="" SIZE="32" MAXLENGTH="256"/>
		</ics:else>
		</ics:if>
		</td>
		<td><img height="1" width="6" src='<%=ics.GetVar("cs_imagedir")+"/graphics/common/screen/dotclear.gif"%>'/></td>
		...
		<td>
			<%if("ucform".equals(ics.GetVar("cs_environment"))){%>
				...
				<ics:then>			
					<ics:setvar name="CVALUE" value='<%=ics.GetVar("ContentDetails:Mapping:"+ curCount + ":value")%>' />
					<INPUT TYPE="HIDDEN" NAME='<%= "CSElement:Mapping:" + curCount + ":value"%>' VALUE='<string:stream variable="CVALUE"/>' SIZE="32" MAXLENGTH="256"/>
				</ics:then>
				<ics:else>
					<ics:removevar name="CVALUE"/>
					<INPUT TYPE="HIDDEN" NAME='<%= "CSElement:Mapping:" + curCount + ":value"%>' VALUE="" SIZE="32" MAXLENGTH="256"/>
				</ics:else>
				</ics:if>
				<input type="hidden" name='<%= "CSElement:Mapping:" + curCount + ":selectedid"%>'/>
				<input type="hidden" name='<%= "CSElement:Mapping:" + curCount + ":selectedtype"%>'/>
				<input type="hidden" name='<%= "CSElement:Mapping:" + curCount + ":selectedname"%>'/>
				<div>
					<div name="_typeAheadElementMapping_<%=curCount%>"> </div>
				</div>
				<script type="text/javascript" language="JavaScript">
					var _selectDnDElementMapping_<%=curCount%> = function(){
						selectFromTreeForMap('<%= "CSElement:Mapping:" + curCount + ":selectedname"%>','<%= "CSElement:Mapping:" + curCount + ":selectedid"%>','<%= "CSElement:Mapping:" + curCount + ":selectedtype"%>','<%=curCount%>');						
					};
				</script>
				<%
			...
			<% } else {%>
               <property:get param="advancedUI.enableAssetForms" inifile="futuretense_xcel.ini" varname="_enableForms"/>
				<%if(null == ics.GetVar("showSiteTree") || "false".equals(ics.GetVar("showSiteTree")) || !"true".equalsIgnoreCase(ics.GetVar("_enableForms"))){
						String sTmpString2 = "Mapping" + curCount;
					%>
					<ics:setvar name="currentUniqueID" value="<%=sTmpString2%>"/>
                    ...
                    <script type="text/javascript">
                        function <%="PickAssetCallback_"+ics.GetVar("currentUniqueID")+"(SelectedAssets)"%>
                        {
                            ...
                            var typeDropDown = document.forms[0].elements['<%="CSElement:Mapping:" + curCount + ":type"%>'];
                            var mappingType = typeDropDown.options[typeDropDown.selectedIndex].value;
                            if (mappingType=="tname")
                            {
                                var totaltltids = [<%=ics.GetVar("totaltltid")%>];
                                ...
                                {
                                    document.forms[0].elements['<%= "CSElement:Mapping:" + curCount + ":value"%>'].value = '/'+AssetInfo[2];
                                }
                                else
                                {
                                    document.forms[0].elements['<%= "CSElement:Mapping:" + curCount + ":value"%>'].value = AssetInfo[2];
                            ...
                            if (mappingType=="assettype")
                            {
                                document.forms[0].elements['<%= "CSElement:Mapping:" + curCount + ":value"%>'].value = AssetInfo[0];
                            }
                            if (mappingType=="assetname")
                            {
                                document.forms[0].elements['<%= "CSElement:Mapping:" + curCount + ":value"%>'].value = AssetInfo[0]+":"+AssetInfo[2];
                            }
                            if (mappingType=="asset")
                            {
                                document.forms[0].elements['<%= "CSElement:Mapping:" + curCount + ":value"%>'].value = AssetInfo[0]+":"+AssetInfo[1];
                            }
                        }
                ...
                <%} else {%>
                    ...
                    <span class="selectorLink"><A HREF="javascript:void(0)" onclick="selectFromTreeForMap('<%= "CSElement:Mapping:" + curCount + ":selectedname"%>','<%= "CSElement:Mapping:" + curCount + ":selectedid"%>','<%= "CSElement:Mapping:" + curCount + ":selectedtype"%>','<%=curCount%>')">
...
<%
		String CSiteid = "CSElement:Mapping:" + curCount + ":siteid";
%>		
...
<INPUT TYPE="hidden" NAME="CSElement:Mapping:Total" Value='<%=total%>' />
</cs:ftcs>