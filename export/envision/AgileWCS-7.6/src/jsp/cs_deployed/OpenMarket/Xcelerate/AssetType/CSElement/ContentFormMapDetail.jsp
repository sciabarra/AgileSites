...
%>
	<ics:callelement element="OpenMarket/Xcelerate/UIFramework/Util/RowSpacer"/>
<%
	String siteid = "CSElement:Mapping:" + curCount + ":siteid";
%>
	<tr>
		<td><img height="1" width="6" src='<%=ics.GetVar("cs_imagedir")+"/graphics/common/screen/dotclear.gif"%>'/></td>
		<td>
		...
		<ics:then>			
			...
			<INPUT TYPE="TEXT" NAME='<%= "CSElement:Mapping:" + curCount + ":key"%>' VALUE='<string:stream variable="CKey"/>' SIZE="20" MAXLENGTH="32"/>
		</ics:then>
		<ics:else>
			<INPUT TYPE="TEXT" NAME='<%= "CSElement:Mapping:" + curCount + ":key"%>' VALUE="" SIZE="20" MAXLENGTH="32"/>
		</ics:else>
		</ics:if>
		</td>
		<td><img height="1" width="6" src='<%=ics.GetVar("cs_imagedir")+"/graphics/common/screen/dotclear.gif"%>'/></td>
