 ...
 *    michele@sciabarra.com - just changed the logger and the package name 
 *******************************************************************************/
...
/**
 * This classloader allows us to close out underlying jars, so that projects can
 * be deleted even if they contain factory jars that are in use.
 * <P>
 * 
 * This classloader caches open jars while it is in use, and once closed it will
 * close those jars. It can still be used after that point, but it will open and
 * close on each classloader operation.
 */
public class JarClassLoader extends ClassLoader {

	final static Log log = Log.getLog(JarClassLoader.class);
	...
	private void open() {
		...
		for (File f : _files) {
			try {
				...
				log.error(ioe, "Unable to create JarFile for file: " + f); //$NON-NLS-1$
			}
		}
	}
	...
	public synchronized void close() {
		...
		for (JarCLInputStream st : _openStreams) {
			try {
				...
				log.error(ioe, "Failed to close stream"); //$NON-NLS-1$
			}
		}
		_openStreams = null;
		...
		for (JarFile jar : _jars) {
			try {
				log.info("**** closing "+jar.getName());
				jar.close();
			} catch (IOException ioe) {
				log.error(ioe, "Failed to close jar: " + jar); //$NON-NLS-1$
			}
		}
		_jars = null;
	}
	...
	public synchronized InputStream getResourceAsStream(String name) {
		...
		for (JarFile j : _jars) {
			try {
				...
				log.error(ioe, "Unable to get entry from jar: " + j); //$NON-NLS-1$
			}
		}
		return null;
	}
		...
		for (JarFile j : _jars) {
			...
			if (entry != null) {
				throw new UnsupportedOperationException(
						"getResource() not implemented: " + name); //$NON-NLS-1$
			}
		}
		return null;
	}
	...
	public Enumeration<URL> getResources(String name) throws IOException {
		throw new UnsupportedOperationException(
				"getResources() not implemented"); //$NON-NLS-1$
	}
	...
	/**
	 * Scan manifest classpath entries of a jar, adding all found jar files to
	 * the set of manifest jars.
	 */
	private static void _recursiveGetManifestJars(File jarFile,
			Set<File> manifestJars) {
		if (!jarFile.exists())
			return;
		...
		try {
			...
			for (String path : rgPaths) {
				...
				// If we haven't seen this, we need to get its manifest jars as
				// well
