...
trait AgileSitesSupport extends AgileSitesUtil {
  ...
  val wcsAssemblyJarTask = wcsAssemblyJar <<=
    (Keys.`package`, wcsShared, streams) map {
      (jar, shared, s) =>
  ...
  val wcsUpdateAssetsTask = wcsUpdateAssets <<=
    (wcsUploadJar, wcsHome, wcsHello, fullClasspath in Compile, wcsUrl, wcsUser, wcsPassword, wcsSites, streams) map {
            ...
            val tomcat = new Thread() {
              override def run() {
                 try {
                   ...
                   val tomcatProcess = tomcatServe(8181, classpath.files, webapps, debug)
